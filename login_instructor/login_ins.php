...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn) {
        ...
        $stmt = $conn->prepare("SELECT id, username, password, status FROM instructor WHERE email = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            // Check if the account is pending or rejected
            if ($user['status'] === 'pending') {
                $loginError = "Your account is pending approval. Please wait for the admin to approve your account.";
            } elseif ($user['status'] === 'rejected') {
                $loginError = "Your account has been rejected. Please contact the administrator.";
            } else {
                // Check password for approved accounts
                if (password_verify($password, $user['password'])) {
                    // Set session variables
                    $_SESSION['username'] = $user['username'];
                    $_SESSION['prof_id'] = $user['id'];  // Store professor ID to track the instructor
                    
                    // Redirect to instructor's dashboard
                    header('Location: ../teacher/dashboard-tt/admin.php');
                    exit();
                } else {
  ...
  .password-container {
    ...
    <div class="cont">      
        ...
        <div class="tre">
            ...
            <a class="but" href="../signup_instructor/signup_ins.php">Sign up now</a>
        </div>
    </div>
